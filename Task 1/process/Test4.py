...
target = int(input("Select the destination number:  "))
print('\n') 
sleep(1)


if target in range(len(unique_addresses_list)):
    src, dst = unique_addresses_list[target]
...
for p in g_packets:
    # filter by source and destination MAC addresses
    if filter_packets(p, src, dst):
        # Store packets with choose destination 
...
if debug in ['y', 'Y']:
    for user_packet, mod_packet in zip([filterd_g_packets[packet_no] for packet_no in packet_nos], mod_packets):
        print("#"*30)
        print("Original Data: ")
        print("#"*30)
...
attack_choice = input("Are you sure to send the modified packets? (time will be changed)[Y/N]: ")
if attack_choice in ['y', 'Y']:
    ...
    try:
        ...
        print("Successfully saved pcapng file as mod_goose_packets.pcapng !")
        print("\n\n")
